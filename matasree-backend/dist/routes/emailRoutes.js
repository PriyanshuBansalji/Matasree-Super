...
router.post('/subscribe', auth_1.verifyToken, async (req, res) => {
    try {
        ...
        let attempts = 0;
        do {
            code = (0, couponRoutes_1.generateUniqueCode)('MS10');
            attempts++;
        } while (await Coupon_1.default.findOne({ code }) && attempts < 5);
...
router.post('/contact', async (req, res) => {
    try {
      ...
      `,
        ...
        // Send both emails ASYNCHRONOUSLY (don't block response)
        Promise.all([
            transporter.sendMail(contactMailOptions).catch(err => console.error('Failed to send contact email:', err.message)),
            transporter.sendMail(replyMailOptions).catch(err => console.error('Failed to send reply email:', err.message)),
        ]).catch(err => console.error('Email sending error:', err));
        return res.status(200).json({
            success: true,
